...
import { normalizeBlogger, type Blogger } from '../blogger-types'
...
export default function CreateDialog({ blogger, onClose, onSave }: CreateDialogProps) {
	const [formData, setFormData] = useState<Blogger>(normalizeBlogger())
	const [showAvatarDialog, setShowAvatarDialog] = useState(false)

	useEffect(() => {
		if (blogger) {
			setFormData(normalizeBlogger(blogger))
		} else {
			setFormData(normalizeBlogger())
		}
	}, [blogger])

	const handleAvatarSubmit = (avatar: AvatarItem) => {
		const avatarUrl = avatar.type === 'url' ? avatar.url : avatar.previewUrl
		setFormData(normalizeBlogger({ ...formData, avatar: avatarUrl }))
	}

	const handleSubmit = () => {
		const normalizedBlogger = normalizeBlogger(formData)

		if (!normalizedBlogger.name.trim() || !normalizedBlogger.avatar.trim() || !normalizedBlogger.url.trim() || !normalizedBlogger.description.trim()) {
			toast.error('请填写所有必填项')
			return
		}

		onSave(normalizedBlogger)
		onClose()
