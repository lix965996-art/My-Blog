...
import { normalizeBlogger, type Blogger, type BloggerStatus } from '../blogger-types'
...
export function BloggerCard({ blogger, isEditMode = false, onUpdate, onDelete }: BloggerCardProps) {
	...
	const { maxSM } = useSize()
	const [localBlogger, setLocalBlogger] = useState(() => normalizeBlogger(blogger))
	const [showAvatarDialog, setShowAvatarDialog] = useState(false)
	const [avatarItem, setAvatarItem] = useState<AvatarItem | null>(null)

	useEffect(() => {
		setLocalBlogger(normalizeBlogger(blogger))
	}, [blogger])

	const handleFieldChange = (field: keyof Blogger, value: any) => {
		const updated = normalizeBlogger({ ...localBlogger, [field]: value })
	...
	const handleAvatarSubmit = (avatar: AvatarItem) => {
		...
		const updated = normalizeBlogger({ ...localBlogger, avatar: avatarUrl })
		setLocalBlogger(updated)
