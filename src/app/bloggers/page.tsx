...
import { normalizeBlogger, normalizeBloggerList, type Blogger } from './blogger-types'

const initialBloggers = normalizeBloggerList(initialList)
...
export default function Page() {
	...
	const handleUpdate = (updatedBlogger: Blogger, oldBlogger: Blogger, avatarItem?: AvatarItem) => {
		const normalizedBlogger = normalizeBlogger(updatedBlogger)
		setBloggers(prev => prev.map(b => (b.url === oldBlogger.url ? normalizedBlogger : b)))
		if (avatarItem) {
			setAvatarItems(prev => {
				const newMap = new Map(prev)
				newMap.set(normalizedBlogger.url, avatarItem)
				return newMap
			})
		}
	}
	...
	const handleSaveBlogger = (updatedBlogger: Blogger) => {
		const normalizedBlogger = normalizeBlogger(updatedBlogger)
		if (editingBlogger) {
			const updated = bloggers.map(b => (b.url === editingBlogger.url ? normalizedBlogger : b))
			setBloggers(updated)
		} else {
			setBloggers([...bloggers, normalizedBlogger])
		}
	}
