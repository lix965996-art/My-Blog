...
import { normalizeSnippets } from './snippet-types'

const getRandomSnippet = (list: string[]) => (list.length === 0 ? '' : list[Math.floor(Math.random() * list.length)])

export default function Page() {
	const normalizedInitialSnippets = normalizeSnippets(initialList)
	const [snippets, setSnippets] = useState<string[]>(normalizedInitialSnippets)
	const [originalSnippets, setOriginalSnippets] = useState<string[]>(normalizedInitialSnippets)
	const [currentSnippet, setCurrentSnippet] = useState<string>(getRandomSnippet(normalizedInitialSnippets))
	...
	useEffect(() => {
		setCurrentSnippet(prev => (prev && snippets.includes(prev) ? prev : getRandomSnippet(snippets)))
	}, [snippets])
	...
	const applyManageChanges = () => {
		const cleaned = normalizeSnippets(draftSnippets)
		if (cleaned.length === 0) {
