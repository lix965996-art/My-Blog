...
type NormalizedSocialButtonConfig = Omit<SocialButtonConfig, 'type'> & {
...
export default function SocialButtons() {
	...
	const sortedButtons = useMemo(() => {
		...
		return buttons
			.map(button => {
				const normalizedType = normalizeSocialButtonType(button)
				if (!normalizedType) {
					console.warn(`[SocialButtons] Unsupported button type "${button.type}" for id "${button.id}"`)
					return null
				}
			...
			.filter((button): button is NormalizedSocialButtonConfig => button !== null)
	...
	const renderButton = (button: NormalizedSocialButtonConfig) => {
