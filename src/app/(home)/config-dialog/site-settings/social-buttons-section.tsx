...
import { normalizeSocialButtonType, type SocialButtonType } from '../../social-button-types'
...
export function SocialButtonsSection({ formData, setFormData, socialButtonImageUploads, setSocialButtonImageUploads }: SocialButtonsSectionProps) {
	...
	const sortedButtons = [...buttons]
		...
		.map(button => ({
			...button,
			normalizedType: normalizeSocialButtonType(button) ?? 'link'
		}))
			...
			<div className='space-y-2 whitespace-nowrap'>
				{sortedButtons.map((button, index) => (
					<div key={button.id} className='flex items-center gap-2'>
						...
						/>
						{button.normalizedType === 'wechat' || button.normalizedType === 'qq' ? (
								...
								{socialButtonImageUploads[button.id]?.type === 'file' ? (
									<div className='relative flex flex-1 items-center gap-2'>
										...
										<input
											...
											placeholder={button.normalizedType === 'wechat' ? '微信号或二维码链接' : 'QQ号或二维码链接'}
							...
							<input
								type={button.normalizedType === 'email' ? 'email' : 'url'}
								value={button.value}
								onChange={e => handleUpdateButton(button.id, { value: e.target.value })}
								placeholder={button.normalizedType === 'email' ? 'example@email.com' : 'https://example.com'}
								className='bg-secondary/10 flex-1 rounded-lg border px-3 py-1.5 text-xs'
							/>
						)}
						{button.normalizedType !== 'email' && button.normalizedType !== 'wechat' && button.normalizedType !== 'qq' && (
							<input
								type='text'
								value={button.label || ''}
