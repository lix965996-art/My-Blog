...
export async function pushPictures(params: PushPicturesParams): Promise<void> {
	...
	const pictures = normalizePictureList(params.pictures)
		...
		for (const [key, imageItem] of imageItems.entries()) {
			if (imageItem.type === 'file') {
				...
				updatedPictures = updatedPictures.map(p => {
					...
					const nextImages = p.images.map((img, idx) => (idx === imageIndex ? publicPath : img))
	...
	if (previousListJson) {
		try {
			const previousPictures = normalizePictureList(JSON.parse(previousListJson))
