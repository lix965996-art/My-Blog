...
import { normalizeShare, normalizeShareList, type Share } from './share-types'

const initialShares = normalizeShareList(initialList)

export default function Page() {
	const [shares, setShares] = useState<Share[]>(initialShares)
	const [originalShares, setOriginalShares] = useState<Share[]>(initialShares)
	...
	const handleUpdate = (updatedShare: Share, oldShare: Share, logoItem?: LogoItem) => {
		const normalizedShare = normalizeShare(updatedShare)
		setShares(prev => prev.map(s => (s.url === oldShare.url ? normalizedShare : s)))
		if (logoItem) {
			setLogoItems(prev => {
				const newMap = new Map(prev)
				newMap.set(normalizedShare.url, logoItem)
				return newMap
			})
		}
	}
	...
	const handleSaveShare = (updatedShare: Share) => {
		const normalizedShare = normalizeShare(updatedShare)
		if (editingShare) {
			const updated = shares.map(s => (s.url === editingShare.url ? normalizedShare : s))
			setShares(updated)
		} else {
			setShares([...shares, normalizedShare])
		}
	}
