...
import { normalizeShare, type Share } from '../share-types'
...
export default function CreateDialog({ share, onClose, onSave }: CreateDialogProps) {
	const [formData, setFormData] = useState<Share>(normalizeShare())
	...
	useEffect(() => {
		if (share) {
			...
			setFormData(normalizedShare)
			setTagsInput(normalizedShare.tags.join(', '))
		} else {
			setFormData(normalizeShare())
			setTagsInput('')
		}
	}, [share])
	...
	const handleLogoSubmit = (logo: LogoItem) => {
		...
		setFormData(normalizeShare({ ...formData, logo: logoUrl }))
	}
	...
	const handleTagsChange = (value: string) => {
		...
		const tags = value
			...
			.map(t => t.trim())
			.filter(t => t)
		setFormData(normalizeShare({ ...formData, tags }))
	}

	const handleSubmit = () => {
		const normalizedShare = normalizeShare(formData)

		if (!normalizedShare.name.trim() || !normalizedShare.logo.trim() || !normalizedShare.url.trim() || !normalizedShare.description.trim()) {
			toast.error('请填写所有必填项')
			return
		}

		if (normalizedShare.tags.length === 0) {
			toast.error('请至少添加一个标签')
			return
		}

		onSave(normalizedShare)
		onClose()
