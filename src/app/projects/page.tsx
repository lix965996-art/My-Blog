...
import { normalizeProject, normalizeProjectList, type Project } from './project-types'

const initialProjects = normalizeProjectList(initialList)
...
export default function Page() {
	...
	const handleUpdate = (updatedProject: Project, oldProject: Project, imageItem?: ImageItem) => {
		...
		setProjects(prev => prev.map(p => (p.url === oldProject.url ? normalizedProject : p)))
		if (imageItem) {
			setImageItems(prev => {
				const newMap = new Map(prev)
				newMap.set(normalizedProject.url, imageItem)
				return newMap
			})
		}
	}
	...
	const handleSaveProject = (updatedProject: Project) => {
		const normalizedProject = normalizeProject(updatedProject)
		if (editingProject) {
			const updated = projects.map(p => (p.url === editingProject.url ? normalizedProject : p))
			setProjects(updated)
		} else {
