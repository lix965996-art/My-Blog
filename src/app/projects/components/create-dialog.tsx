...
export default function CreateDialog({ project, onClose, onSave }: CreateDialogProps) {
	const [formData, setFormData] = useState<Project>(normalizeProject())
	...
	useEffect(() => {
		if (project) {
			...
			setTagsInput(normalizedProject.tags.join(', '))
		} else {
			setFormData(normalizeProject())
			setTagsInput('')
		}
	}, [project])
	...
	const handleImageSubmit = (image: ImageItem) => {
		...
		setFormData(normalizeProject({ ...formData, image: imageUrl }))
	}
	...
	const handleTagsChange = (value: string) => {
		...
		const tags = value
			...
			.map(t => t.trim())
			.filter(t => t)
		setFormData(normalizeProject({ ...formData, tags }))
	}

	const handleSubmit = () => {
		const normalizedProject = normalizeProject(formData)

		if (!normalizedProject.name.trim() || !normalizedProject.image.trim() || !normalizedProject.url.trim() || !normalizedProject.description.trim()) {
			toast.error('请填写所有必填项')
			return
		}

		if (normalizedProject.tags.length === 0) {
			toast.error('请至少添加一个标签')
			return
		}

		onSave(normalizedProject)
		onClose()
