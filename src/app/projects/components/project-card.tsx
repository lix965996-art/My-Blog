...
export function ProjectCard({ project, isEditMode = false, onUpdate, onDelete }: ProjectCardProps) {
	...
	const { maxSM } = useSize()
	const [localProject, setLocalProject] = useState(() => normalizeProject(project))
	const [showImageDialog, setShowImageDialog] = useState(false)
	const [imageItem, setImageItem] = useState<ImageItem | null>(null)

	useEffect(() => {
		setLocalProject(normalizeProject(project))
	}, [project])

	const handleFieldChange = (field: keyof Project, value: any) => {
		const updated = normalizeProject({ ...localProject, [field]: value })
	...
	const handleImageSubmit = (image: ImageItem) => {
		...
		const updated = normalizeProject({ ...localProject, image: imageUrl })
		setLocalProject(updated)
